...
```{r}
predict.lasso.test2 = predict.lasso.test %>% 
  as.data.frame() %>% 
  mutate(predict.lasso.test, candidate = factor(ifelse(predict.lasso.test[,1] == "0", "Donald Trump", "Hillary Clinton"))) %>% 
  select(candidate)
...
pred.lasso = prediction(as.numeric(predict.lasso.test2$candidate), as.numeric(tst.cl$candidate))
