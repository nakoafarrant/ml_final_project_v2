...
```
<<<<<<< HEAD
Using boosting, white and transit now become the top two most important variables. The above plots tell us that higher proportion of country transit ridership is negatively correlated with Trump winning the vote while higher proportions of white populations in a county is positively correlated with Trump winning the vote. 

=======
Using boosting, White and Transit now become the top two most important variables. The above plots tell us that higher proportion of country transit ridership is negatively correlated with Trump winning the vote while higher proportions of white populations in a county is positively correlated with Trump winning the vote.
>>>>>>> fc09507cfe6463490408fcaf5c82b69084363c58
...
```
The test error rate for boosting, `r round(test.boost.err, 4)`, is higher than that of bagging and random forest. 
...
```
The test error rate for support vector model, `r round(test.svm.err, 4)`, is very similar to that of the random forest model and lower than the boosting model. 
...
testmodels[1,1] = calc_error_rate(predict.tree.test, tst.cl$candidate)
testmodels[2,1] = calc_error_rate(predict.logistic.test$candidate, tst.cl$candidate)
testmodels[3,1] = calc_error_rate(predict.lasso.test, y.tst)
